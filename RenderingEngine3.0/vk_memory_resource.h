...
namespace rcq
{
	...
	public:
		...
		void init(VkDevice device, uint32_t memory_type_index, bool map, const vk_allocator* vk_alloc)
		{
			device_memory_resource::init(std::numeric_limits<VkDeviceSize>::max(), device, VK_NULL_HANDLE, nullptr);
		...
		uint64_t allocate(uint64_t size, uint64_t alignment = 0) override
		{
			...
			assert(vkAllocateMemory(m_device, &alloc, *m_vk_alloc, &m_handle)==VK_SUCCESS);
