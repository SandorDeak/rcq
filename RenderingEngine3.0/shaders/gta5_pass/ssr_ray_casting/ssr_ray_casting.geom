...
layout(set=0, binding=1) uniform sampler2D pos_tex;

layout(set=0, binding=2) uniform ssr_ray_casting_data
{
	mat4 proj_x_view;
	vec3 view_pos;
	float ray_length;
...
void main()
{
	...
	fragment_id_out=fragment_id_in[0];
	gl_Position=data.proj_x_view*vec4(pos, 1.f);
	EmitVertex();
	
	fragment_id_out=vec2(fragment_id_in[0]);
	gl_Position=data.proj_x_view*vec4(pos+data.ray_length*r, 1.f); 
	EmitVertex();	
}